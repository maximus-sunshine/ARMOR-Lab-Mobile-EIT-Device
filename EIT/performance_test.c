...
int main()
{
	...
	***********************************/
	...
	//gettimeofday(&t1, NULL);	//t1 for avg. cycle
	while(count < cycles){
		count++;
		gettimeofday(&t1, NULL); //t1 for last cycle
		// printf("\n\n\n******************** Cycle %d *************************\n\n",count);
	...
	printf("\n\n DONE SAMPLING %d nodes, %d cycles. Last cycle took %0.5f seconds. Frequency of last cycle: %0.5f\n",NODAL_NUM, cycles, usec/1e6, 1/(usec/1e6));
	fflush(stdout);
