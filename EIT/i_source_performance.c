 ...
 *	- Jacob Rutheiser
 *
 * Current source performance test for Rev02.	 
 *
 * compile with "gcc -pthread i_source_performance.c src/eit.c src/gpiolib.c src/ti-ads8684.c -o i_source_performance"
 *
 ************************************************************************************/
...
int current_setpoint = 4; //current setpoint 100uA-2000uA(0-19, 100uA) TODO, make this better
	...
	while(count < cycles){
		...
		for(i = 0; i < NODAL_NUM; i++){
			...
			for(j = 0; j < (NODAL_NUM-2); j++){
		        ...
		        printf("%0.9f\t%0.9f\n",chan0*scale/1000,chan1*scale/1000);
		        fflush(stdout);
